...
async function getStats(username, pushHistory, onError) {
  ...
  if (!res.code) {
    ...
    data.id = res.id;
    data.profile.profilePicUrl = `https://cdn2.scratch.mit.edu/get_image/user/${data.id}_120x120.png`;
...
async function getActivityData() {
  ...
  const req = await fetch(`https://api.scratchstats.com/scratch/users/${data.user}/activity`);
  requestMade();
  if(req.status === 200) {
